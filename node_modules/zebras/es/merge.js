...
import { keys, filter, includes, intersection, groupBy, prop, difference } from "ramda";
...
var fillRow = function fillRow(row, cols) {
  var rowCols = keys(row);
  var filledRow = row;
  difference(cols, rowCols).forEach(function (c) {
...
var merge = function merge(dfLeft, dfRight, leftOn, rightOn, leftSuffix, rightSuffix) {
  var colsLeft = keys(dfLeft[0]);
  var colsRight = keys(dfRight[0]);
  var intersected = filter(function (x) {
    return !includes(x, [leftOn, rightOn]);
  ...
  var colsLeftUpdated = keys(dfLeftUpdated[0]);
  var colsRightUpdated = keys(dfRightUpdated[0]);
  var colsAll = Array.from(new Set([].concat(_toConsumableArray(colsLeftUpdated), _toConsumableArray(colsRightUpdated))));
  var dfLeftGrouped = groupBy(prop(leftOn), dfLeftUpdated);
  var dfRightGrouped = groupBy(prop(rightOn), dfRightUpdated);
  var index = keys(dfLeftGrouped);
